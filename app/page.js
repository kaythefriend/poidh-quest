...
export default function Home() {
  ...
  useEffect(() => {
    fetch('/api/bounties').then(async (r) => { const json = await r.json(); if (!r.ok) throw new Error(json.error || 'Unable to load quests'); return json; }).then(setData).catch((e) => setError(e.message)).finally(() => setLoading(false));
    fetch('/api/stats').then((r) => r.json()).then((json) => { if (!json.error) setStats(json); }).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      for (let i = 0; i < data.length; i += 8) {
        const results = await Promise.all(data.slice(i, i + 8).map(async (bounty) => {
          try {
            const slug = chainNames[bounty.chainId];
            const r = await fetch(`https://poidh.xyz/${slug}/bounty/${bounty.id}/data`, { cache: 'no-store' });
            if (!r.ok) return null;
            const d = await r.json();
            return [bounty.id + '-' + bounty.chainId, { priceUsd: d.priceUsd, submissions: Array.isArray(d.claims) ? d.claims.length : null, image: d.claims?.find((c) => c.imageUrl)?.imageUrl || null, title: d.title, description: d.description }];
          } catch { return null; }
        }));
        if (cancelled) return;
        setDetails((prev) => { const next = { ...prev }; results.filter(Boolean).forEach(([key, value]) => { next[key] = value; }); return next; });
      }
    };
    load();
  ...
  const rows = useMemo(() => {
    ...
    return data.filter((b) => {
      ...
      if (sort === 'reward') return Number(db.priceUsd ?? b.priceUsd ?? 0) - Number(da.priceUsd ?? a.priceUsd ?? 0);
  ...
  return <main>
    <header className="hero">
      ...
      <div className="controls">
        ...
        <select value={chain} onChange={(e) => { setChain(e.target.value); setVisible(60); }}><option value="all">All chains</option><option value="mainnet">Ethereum</option><option value="arbitrum">Arbitrum</option><option value="base">Base</option><option value="degen">Degen</option></select>
    ...
    <section className="grid">{shown.map((b) => {
