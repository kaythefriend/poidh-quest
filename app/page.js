...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      for (let i = 0; i < data.length; i += 8) {
        ...
        const results = await Promise.all(batch.map(async (bounty) => {
          try {
  ...
  return <main>
    <header className="hero">
      ...
      <div className="stats"><b>{data.length || '—'} <small>active quests</small></b><b>4 <small>networks</small></b><b>100% <small>POIDH data</small></b></div>
